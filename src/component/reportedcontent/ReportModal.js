...
const   ReportModal = ({ report, onClose, refreshReports }) => {
  ...
  const handleAction = async (action) => {
    try {
      const res = await fetch(`${API_URL}/admin/report/block/${report._id}`, {
        ...
        body: JSON.stringify({ status: action === 'approve' ? 'Reviewed' : 'Blocked' }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4 py-10">
      <div className="relative bg-white/90 backdrop-blur-lg border border-white/40 rounded-3xl shadow-2xl max-w-xl w-full p-8 animate-scaleIn">
        <button
          onClick={onClose}
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm font-medium capitalize text-gray-800">{status}</span>
          </div>
          <span className="text-sm text-gray-600">Reported on: {formatDate(createdAt)}</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
          {[reportedUser, reporter].map((user, idx) => (
            ...
            >
              ...
              />
              <h3 className="font-semibold text-lg text-gray-800 mt-3">{user?.name || user?.fullName || 'Unnamed User'}</h3>
        ...
        {image?.length > 0 && (
          <div className="mb-4">
            <h4 className="text-sm font-semibold text-gray-800 mb-2">Attached Screenshot{image.length > 1 ? 's' : ''}:</h4>
